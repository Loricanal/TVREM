...
classifiers = [
    ...
    #[SVC(probability=True),[{'kernel': ['rbf'], 'gamma': [1e-3, 1e-4],
    #                 'C': [1, 10, 100, 1000]},
...
regr = set([str(r).split('(')[0] for r in regressors])
...
def train(train_df,train_index,val_df,val_index,test_df,test_index,path,c,r):
    cols = [c for c in list(train_df.columns) if c != 'Y']
    path_scores = path + "score_obj.p"
    path_model = path + "model.h5"
    path_setting = path + "r.p"
    ...
    try:
        ...
        #print(22)
        score_obj = {
            "accb_train":computeAccBal(X_train,Y_train,M),
            "accb_test":computeAccBal(X_test,Y_test,M),
        ...
        pickle.dump(r,open(path_setting,"wb"))
        return 1
    except:
        return 0
        ...
        for feat in ["all","sim","un_int_card"]:
            ...
            if r["sampling"] != "over":
                ...
                if r_key not in done_keys:
                    ...
                    val = train(train_df,train_index,val_df,val_index,test_df,test_index,path,a,r)
                    if val == 1:
                        done_keys.add(r_key)
